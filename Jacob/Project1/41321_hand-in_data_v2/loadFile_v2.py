...
class shape():
    def __init__(self):
        ...
        self.rho = None
        self.name = None
        self.drop: DropCalculations | None = None
    ...
    @property
    def C_A(self):
        if self._C_A is None:
            self._C_A = 4*self.g*(self.s-1)*self.d_A/(3*self.omega_s**2)
        return self._C_A
    ...
    def get_values(self):
        def rounder(x):
            offset = 5 - math.floor(math.log10(abs(x)))
        ...
        m_omega_s = np.mean(self.omega_s)
        s_omega_s = np.std(self.omega_s)
        m_Re_n = np.mean(self.Re_n)
        m_C_D = np.mean(self.C_D)
        m_C_n = np.mean(self.C_n)
        m_C_A = np.mean(self.C_A)
        m_C_D_theo = np.mean(self.C_D_theo)

        # return [self.V, self.A, self.P_p, self.d_D, self.d_n, self.d_A, m_omega_s, s_omega_s, m_Re_n, m_C_D, m_C_n, m_C_A, m_C_D_theo]
        return [rounder(self.V), rounder(self.A), rounder(self.P_p), rounder(self.d_D), rounder(self.d_n), rounder(self.d_A), rounder(m_omega_s), rounder(s_omega_s), rounder(m_Re_n), rounder(m_C_D), rounder(m_C_n), rounder(m_C_A), rounder(m_C_D_theo)]
...
class SingleDropCalculations():
    ...
    def _gauss_interpolate(self, x):
        """ Return fractional peak position assuming Guassian shape
            ...
            Author: KEM from the LDA experiment in 41882
        """
...
class DropCalculations():
    def __init__(self, file_path, shape_name):
        ...
        self.fig_path = r"C:\Users\jacob\OneDrive - Danmarks Tekniske Universitet\Complex flows\HandIn1\Figures"
    ...
    def do_spectral_analysis_ori(self):
        ...
        for idx, (ori, ori_str) in enumerate([(self.x_ori, "x orientation"), (self.y_ori, "y orientation"), (self.z_ori, "z orientation")]):
            ...
            # Plot Spectrum
            if not ori_str == "z orientation" and self._shape_name == "Rectangle_2.5_10_5":
                plt.loglog(f[1:self.N//2], sigmean[1:self.N//2], label="{} - Peak at {:.2f} Hz".format(ori_str, freq), alpha=0.8)
            else:
                plt.loglog(f[1:self.N//2], sigmean[1:self.N//2], label="{}".format(ori_str), alpha=0.8)
        ...
        plt.tight_layout()
        fig_save_path = os.path.join(self.fig_path, self._shape_name + '_spectrum_ori.pdf')
    ...
    def do_spectral_analysis(self):
        ...
        for idx, (vel, vel_str) in enumerate([(self.x_vel_filtered, "x velocity"), (self.y_vel_filtered, "y velocity"), (self.z_vel_filtered, "z velocity")]):
            ...
            # Plot Spectrum
            if not vel_str == "z velocity" and self._shape_name == "Rectangle_2.5_10_5":
                plt.loglog(f[1:self.N//2], sigmean[1:self.N//2], label="{} - Peak at {:.2f} Hz".format(vel_str, freq), alpha=0.8)
            else:
                plt.loglog(f[1:self.N//2], sigmean[1:self.N//2], label="{}".format(vel_str), alpha=0.8)
        ...
        plt.tick_params(axis='both', which='major', labelsize=13)  
        plt.tight_layout()
        fig_save_path = os.path.join(self.fig_path, self._shape_name + '_spectrum_vel.pdf')
        plt.savefig(fig_save_path)
    ...
    def plot_speed_vs_orientation(self):
        ...
        ax.grid()
        for i in range(len(self.drops)):
            #Standardize the y orientation and the z velocity
            # y_ori = (self.y_ori_rel_to_first[:,i] - np.mean(self.y_ori_rel_to_first[:,i]))/np.std(self.y_ori_rel_to_first[:,i])
            # z_vel = (np.abs(self.z_vel_filtered[:,i]) - np.mean(np.abs(self.z_vel_filtered[:,i])))/np.std(np.abs(self.z_vel_filtered[:,i]))
            # ax.scatter(y_ori, z_vel, alpha=.3, s=15)
            ax.scatter(self.y_ori_rel_to_first[:,i], np.abs(self.z_vel_filtered[:,i]), alpha=.2, s=15)
        ax.set_xlabel('y orientation')
        ax.set_ylabel('Absolute z velocity')
        fig.tight_layout()
        fig_save_path = os.path.join(self.fig_path, self._shape_name + '_speed_vs_orientation_uf.pdf')
        plt.savefig(fig_save_path)
        ...
        for i in range(len(self.drops)):
            #Standardize the y orientation and the z velocity
            y_ori += list((self.y_ori_rel_to_first[:,i] - np.mean(self.y_ori_rel_to_first[:,i]))/np.std(self.y_ori_rel_to_first[:,i]))
            z_vel += list((np.abs(self.z_vel_filtered[:,i]) - np.mean(np.abs(self.z_vel_filtered[:,i])))/np.std(np.abs(self.z_vel_filtered[:,i])))
        h = ax.hist2d(y_ori, z_vel, bins=50)
        fig.colorbar(h[3], ax=ax, label='Number of points in bin')
        # Add colorbar
        ax.set_xlabel('Standardized y orientation')
        ax.set_ylabel('Standardized z velocity')
        # ax.set_title('Speed vs orientation')
        fig.tight_layout()
        fig_save_path = os.path.join(self.fig_path, self._shape_name + '_speed_vs_orientation_hist.pdf')
        plt.savefig(fig_save_path)
        stop = True
...
if __name__ == '__main__':
    ...
    if False:
        ...
        cube_values = cube_test.get_values()
        square_values = square_test.get_values()
        rectangular_values = rectangular_test.get_values()

        # Save in a dataframe
        df = pd.DataFrame([cube_values, square_values, rectangular_values], index=["Cube_5mm", "4x4x8", "Rectangle_2.5_10_5"])
        # Save to csv
        path = os.path.join(r"C:\Users\jacob\OneDrive - Danmarks Tekniske Universitet\Complex flows\HandIn1", "shape_values.csv")
        df.to_csv(path, sep = ';')

        stop = True

    for shape_name in ["Cube_5mm", "4x4x8", "Rectangle_2.5_10_5"]:
