...
for track_number in range(10):
    ...
    fft_output_x = np.fft.rfft(velocitiy_x)
    fft_output_y = np.fft.rfft(velocitiy_y)
    fft_output_z = np.fft.rfft(velocitiy_z)
    fft_output_rotx = np.fft.rfft(velocitiy_rotx)
    fft_output_roty = np.fft.rfft(velocitiy_roty)
    fft_output_rotz = np.fft.rfft(velocitiy_rotz)
...
freqs = np.fft.rfftfreq(len(velocitiy_x),sample_spacing)

f = np.arange(len(velocitiy_x) // (len(velocitiy_x) * sample_spacing))    
...
#plt.figure()
#plt.loglog(f, fft_plot_x, label='x')
#plt.loglog(f, fft_plot_y, label='y')
#plt.loglog(f, fft_plot_z, label='z')
#plt.loglog(f, fft_plot_rotx, label='rotx')
#plt.loglog(f, fft_plot_roty, label='roty')
#plt.loglog(f, fft_plot_rotz, label='rotz')
#plt.legend()
#plt.xlabel("time (s)")
#plt.ylabel("Voltage (mV)")
#plt.title(shape_name)
